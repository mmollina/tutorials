...
```{r, all_mat_rf, eval=TRUE, echo = TRUE, results='hide'}
...
```{r print_maps, results='hide', eval = TRUE}
w<-NULL
  for(i in 1:12)
  w<-c(w, c(MAPs[i],
            MAPs.prior[i], 
...
```{r hmm_map_mds, results='hide', eval = TRUE}
## Performing parallel computation
system.time({
  cl <- parallel::makeCluster(12)
  parallel::clusterEvalQ(cl, require(mappoly))
...
```{r all_maps_error_mds, results='hide', eval = TRUE}
...
Plotting maps
```{r print_maps_mds, results='hide', eval = TRUE}
w<-NULL
  for(i in 1:12)
  w<-c(w, c(MAPs.mds[i],
            MAPs.mds.prior[i], 
